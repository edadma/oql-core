...
@JSExportAll
class QueryBuilder_JS_NodePG private[oql] (private val oql: OQL_NodePG_JS, private[oql] val q: OQLQuery, fixed: Fixed) {
  private def check = if (q.source eq null) sys.error("QueryBuilder: no source was given") else this

  private class DoNothingQueryBuilder extends QueryBuilder_JS_NodePG(oql, q, fixed) {
    private def na = sys.error("not applicable")

    override def cond(v: Any): QueryBuilder_JS_NodePG = na
    ...
    override def limit(a: Int): QueryBuilder_JS_NodePG = QueryBuilder_JS_NodePG.this

    override def offset(a: Int): QueryBuilder_JS_NodePG = QueryBuilder_JS_NodePG.this

    override def order(attribute: String, sorting: String): QueryBuilder_JS_NodePG = QueryBuilder_JS_NodePG.this
...
//    override def add(attribute: QueryBuilder): QueryBuilder = QueryBuilder.this
    ...
    override def query(query: String, parameters: js.UndefOr[js.Any] = js.undefined): QueryBuilder_JS_NodePG =
      QueryBuilder_JS_NodePG.this

    override def select(s: String, parameters: js.UndefOr[js.Any] = js.undefined): QueryBuilder_JS_NodePG =
      QueryBuilder_JS_NodePG.this
  }

  def cond(v: Any): QueryBuilder_JS_NodePG =
...
//    )
  ...
  def query(query: String, parameters: js.UndefOr[js.Any] = js.undefined): QueryBuilder_JS_NodePG =
    new QueryBuilder_JS_NodePG(oql, oql.parseQuery(oql.substitute(query, parameters)), fixed)

  def select(s: String, parameters: js.UndefOr[js.Any] = js.undefined): QueryBuilder_JS_NodePG = {
  ...
  def order(attribute: String, sorting: String): QueryBuilder_JS_NodePG = {
    val attr = AttributeOQLExpression(List(Ident(attribute)), null)

    AbstractOQL.decorate(q.entity, attr, oql.model, oql.ds, null)
    new QueryBuilder_JS_NodePG(oql, q.copy(order = Some(List(OQLOrdering(attr, sorting)))), fixed)
  }

  def limit(a: Int): QueryBuilder_JS_NodePG = new QueryBuilder_JS_NodePG(oql, q.copy(limit = Some(a)), fixed)

  def offset(a: Int): QueryBuilder_JS_NodePG = new QueryBuilder_JS_NodePG(oql, q.copy(offset = Some(a)), fixed)
